...
-- BUG: n and from are swapped in the Deduct branch
-- n :: Integer is used where Withdraw is expected and vice versa
netWithdraw :: Withdraw -> Integer
netWithdraw w = case w of
  Amount n -> n
  Joint x y -> netWithdraw x + netWithdraw y
  Deduct n from -> netWithdraw n - from
