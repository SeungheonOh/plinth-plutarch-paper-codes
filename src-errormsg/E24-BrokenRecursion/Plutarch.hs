...
-- apply the PWithdraw term with # as if it were a function type
netWithdraw :: Term s (PWithdraw :--> PInteger)
netWithdraw = plam $ \netWithdraw -> pmatch netWithdraw $ \w ->
  case w of
    PAmount n -> pfromData n
    PJoint x y -> (netWithdraw # pfromData x) + (netWithdraw # pfromData y)
    PDeduct n from -> (netWithdraw # pfromData from) - pfromData n
