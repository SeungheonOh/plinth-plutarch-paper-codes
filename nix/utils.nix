...
rec {
  flattenDerivationTree = separator: set:
    let
      ...
      flatten = name': value:
        let name = builtins.replaceStrings [ ":" ] [ separator ] name';
        in if lib.isDerivation value || lib.typeOf value != "set" then [{
          inherit name value;
        }] else
          lib.concatLists (lib.mapAttrsToList (recurse name) value);
    in assert lib.typeOf set == "set"; lib.listToAttrs (flatten "" set);
